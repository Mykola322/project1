...
from .models import Author, Book, Publisher, Bug
...
class BugSerializer(serializers.ModelSerializer):
    ...
    def validate_severity(self, value):
        if not 1 <= value <= 5:
            raise serializers.ValidationError("Рівень критичності має бути від 1 до 5")
        return value
    

    def validate(self, data):
        if data.get('status') == 'resolved' and not data.get('description'):
            raise serializers.ValidationError("Баг не може бути вирішеним без опису")
        return data